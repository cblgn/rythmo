        ...
        thread(name = "rythmo-discovery", isDaemon = true) {
            while (!socket.isClosed) {
                try {
                    ...
                    if (String(packet.data, 0, packet.length, Charsets.UTF_8) == "RYTHMO/3?") {
                        val bytes = "RYTHMO/3 $serverId $httpPort".toByteArray()
...
fun discoverTeachers(): List<String> {
    ...
    DatagramSocket().use { socket ->
        ...
        val query = "RYTHMO/3?".toByteArray()
        ...
        while (System.nanoTime() < deadline) {
            try {
                ...
                if (fields.size == 3 && fields[0] == "RYTHMO/3" && fields[2].toIntOrNull() in 1..65535) {
                    found += "https://${packet.address.hostAddress}:${fields[2]}"
                }
